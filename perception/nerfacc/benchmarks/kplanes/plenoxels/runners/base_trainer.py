...
class BaseTrainer(abc.ABC):
    ...
    def _normalize_depth(
        ...
        return (self._normalize_01(depth)).cpu().reshape(img_h, img_w)[..., None]
    ...
    def calc_metrics(self, preds: torch.Tensor, gt: torch.Tensor):
        ...
        return {
            ...
            "alex_lpips": metrics.rgb_lpips(
                preds, gt, net_name="alex", device=err.device
            ),
            "vgg_lpips": metrics.rgb_lpips(
                preds, gt, net_name="vgg", device=err.device
            ),
        }
    ...
    def report_test_metrics(
        self, scene_metrics: Dict[str, Sequence[float]], extra_name: Optional[str]
    ):
    ...
    def init_optim(self, **kwargs) -> torch.optim.Optimizer:
        ...
        if optim_type == "adam":
            optim = torch.optim.Adam(
                params=self.model.get_params(kwargs["lr"]), eps=1e-8
            )
        else:
...
def initialize_model(
    runner: Union["StaticTrainer", "PhototourismTrainer", "VideoTrainer"], **kwargs
) -> LowrankModel:
