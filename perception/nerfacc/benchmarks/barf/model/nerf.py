...
class Model(base.Model):
    ...
    @torch.no_grad()
    def visualize(self, opt, var, step=0, split="train", eps=1e-8):
        if opt.tb:
    ...
    @torch.no_grad()
    def evaluate_full(self, opt, eps=1e-8):
        self.graph.eval()
    ...
    @torch.no_grad()
    def generate_videos_synthesis(self, opt, eps=1e-8):
        self.graph.eval()
    ...
    def render(self, opt, pose, intr=None, ray_idx=None, mode=None):
        ...
        if self.nerf.use_occ_grid:
            ...
            def sigma_fn(t_starts, t_ends, ray_indices):
                ...
                return self.nerf(opt, positions, t_dirs, mode=mode, density_only=True)

            def rgb_sigma_fn(t_starts, t_ends, ray_indices):
                if ray_indices.shape[0] == 0:
                    return torch.zeros((0, 3), device=ray_indices.device), torch.zeros(
                        (0,), device=ray_indices.device
                    )
                t_origins = flatten_center[ray_indices]
                t_dirs = flatten_ray[ray_indices]
                positions = t_origins + t_dirs * (t_starts + t_ends)[:, None] / 2.0
            ...
            rgb, opacity, depth = [
                t.reshape(batch_size, -1, t.shape[-1]) for t in [rgb, opacity, depth]
            ]
        else:
...
class NeRF(torch.nn.Module):
    def __init__(self, opt):
        ...
        self.occ_grid = None
        self.occ_aabb = opt.nerf.get("occ_aabb", [-1.5, -1.5, -1.5, 1.5, 1.5, 1.5])
