...
class OccGridEstimator(AbstractEstimator):
    ...
    def mark_invisible_cells(
        ...
        invis_map = torch.clone(self.occs.detach())
        ...
        for lvl, indices in enumerate(lvl_indices):
            ...
            for i in range(0, len(indices), chunk):
                ...
                uvd[:, 2] = -uvd[:, 2]  # flip z to be consistent with the camera space
                in_image = (
                    (uvd[:, 2] >= 0)
                    & (uv[:, 0] >= 0)
                    & (uv[:, 0] < width)
                ...
                # if depth available, we only mark the cells that are in front of the wall/obstacle

                depth_values = torch.zeros(uvd[:, 2].shape).to(uvd.device)
                for cam_id in range(uvd[:, 2].shape[0]):
                    width_id = torch.clamp(uv[cam_id, 0, :], 0, width - 1).to(torch.int)
                    height_id = torch.clamp(uv[cam_id, 1, :], 0, height - 1).to(
                        torch.int
                    )
                    depth_values[cam_id] = depth[cam_id, width_id, height_id]

                # st()
                covered_by_cam &= uvd[:, 2] < torch.clamp(
                    (depth_values - 0.1), min=0
                )  # torch.clamp((depth_values + 0.2), min=0.5)
                ...
                invis_map[cell_ids_base + indices_chunk] = torch.where(
                    valid_mask, 0.0, -1.0
                )
        return invis_map
    ...
    def _update(
        ...
        for lvl, indices in enumerate(lvl_indices):
            ...
            # TODO Filter out invisible (-1) No need
            # cell_ids = cell_ids[self.occs[cell_ids] >= 0.0]
