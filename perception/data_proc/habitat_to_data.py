...
class Dataset(torch.utils.data.Dataset):
    ...
    def remove_data(self, index):
        st()
        self.images = torch.cat([self.images[:index], self.images[index + 1 :]], dim=0)
        self.depths = torch.cat([self.depths[:index], self.depths[index + 1 :]], dim=0)
        self.semantics = torch.cat(
            [self.semantics[:index], self.semantics[index + 1 :]], dim=0
        )
        self.camtoworlds = torch.cat(
            [self.camtoworlds[:index], self.camtoworlds[index + 1 :]], dim=0
        )
