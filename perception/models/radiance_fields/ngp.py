...
class NGPRadianceField(torch.nn.Module):
        ...
        if self.geo_feat_dim > 0:
            self.mlp_head = tcnn.Network(
                ...
                network_config={
                    ...
                    "activation": "ReLU",
                    "output_activation": "None",
                    "n_neurons": int(neurons / sem_neuron_scale),
                    "n_hidden_layers": 2,
                },
            )

            if num_semantic_classes > 0:
                self.mlp_sem = tcnn.Network(
                    n_input_dims=self.geo_feat_dim,
                    n_output_dims=num_semantic_classes,
                    network_config={
                        "otype": "FullyFusedMLP",
                        "activation": "ReLU",
                        "output_activation": "None",
    ...
    def _query_semantic(self, embedding):
        return (
            self.mlp_sem(embedding.reshape(-1, self.geo_feat_dim))
            .reshape(list(embedding.shape[:-1]) + [self.num_semantic_classes])
            .to(embedding)
        )
    ...
    def forward(
        ...
        if self.use_viewdirs and (directions is not None):
            ...
            if self.num_semantic_classes > 0:
                sem_logits = self._query_semantic(embedding=embedding)
                return rgb, density, sem_logits
    ...
    def semantic_entropy(self):
        if self.num_semantic_classes > 0:
            x = torch.arange(500) / 500 * (self.aabb[3] - self.aabb[0]) + self.aabb[0]
            z = torch.arange(50) / 50 * (self.aabb[4] - self.aabb[1]) + self.aabb[1]
            y = torch.arange(500) / 500 * (self.aabb[5] - self.aabb[2]) + self.aabb[2]
            gridx, gridz, gridy = torch.meshgrid(x, z, y)
            gridx.unsqueeze_(3)
            gridz.unsqueeze_(3)
            gridy.unsqueeze_(3)
            points = torch.flatten(torch.cat((gridx, gridz, gridy), dim=3), 0, 2)
