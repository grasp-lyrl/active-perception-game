...
def sample_traj(
    ...
    path_finding_map = (v_merge > threshold).astype(np.int32)
    # voxel_grid[0, :, :, 8].astype(
    #     np.int32
    # )
    ...
    path_finding_map[v_idx[0], v_idx[1]] = False
    path_finding_map[v_idx[0] + 1, v_idx[1]] = False
    path_finding_map[v_idx[0] - 1, v_idx[1]] = False
    path_finding_map[v_idx[0], v_idx[1] + 1] = False
    path_finding_map[v_idx[0], v_idx[1] - 1] = False

    vm = np.copy(visiting_map)
    vm[(path_finding_map > threshold) == 1] = -1
    vm[(path_finding_map > threshold) == 0] = np.exp(
        -(
            vm[(path_finding_map > threshold) == 0]
            - np.min(vm[(path_finding_map > threshold) == 0])
        )
        / 5
    )
    ...
    fig, ax = plt.subplots()
    sc = ax.imshow(vm, vmin=-1, vmax=1)
    ax.plot(v_idx[1], v_idx[0], "g*")
    fig.colorbar(sc)
    fig.savefig(save_path + "/maps/vmap_" + str(current_time) + ".png")
    fig.clf()
    plt.close(fig)
    ...
    for _ in tqdm.tqdm(range(N_traj)):
        ...
        while in_collision:
            ...
            factor = max(int(len(path[0]) / 40) * 2, 2)

            if c < 0.5:
                yaw = np.linspace(factor * np.pi, 0, len(dij_waypoints))
            else:
                yaw = -np.linspace(factor * np.pi, 0, len(dij_waypoints))
            ...
            N_sample_disc = max(int(t_final * 35), 35)
        ...
        for ang in np.linspace(0, 360, 20):
            ...
            pos[:3] += np.random.uniform([-0.05, -0.05, -0.05], [0.05, 0.05, 0.05], 3)
