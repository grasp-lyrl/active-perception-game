...
class HabitatSim:
    def __init__(self, scene, scene_dataset_config_file, img_w, img_h):
        ...
        #     "/home/siminghe/code/active-perception-game/data/habitat_collection/036_good_model_improved_2/train/data0.npz"
        # )
        # # locs = observations["camtoworlds"][:, :3, 3]
        # # self.add_visited_location(locs, 0.1)
        # st()
    ...
    def make_simple_cfg(self, settings):
        ...
        # sample_rgb_sensor_spec.noise_model = "GaussianNoiseModel"
        # sample_rgb_sensor_spec.noise_model_kwargs = dict(mean=0, sigma=1)
        ...
        # sample_depth_sensor_spec.noise_model = "RedwoodDepthNoiseModel"
        # sample_depth_sensor_spec.noise_model_kwargs = dict(noise_multiplier=5)
    ...
    #     # ds_img = np.array(rgbs)[:, ::8, ::8]
    #     # low_res = np.repeat(np.repeat(ds_img, 8, axis=1), 8, axis=2)

    #     # ds_depth = np.array(depths)[:, ::8, ::8]
    #     # low_res_depth = np.repeat(np.repeat(ds_depth, 8, axis=1), 8, axis=2)

    #     # ds_sem = np.array(sems)[:, ::8, ::8]
    #     # low_res_sem = np.repeat(np.repeat(ds_sem, 8, axis=1), 8, axis=2)

    #     # return np.array(low_res), np.array(low_res_depth), np.array(low_res_sem)
    #     return np.array(rgbs), np.array(depths), np.array(sems)
    ...
    def render_top_tpv(self, poses, aabb, draw_traj=True):
        """
        move the quad and retrieve third-person view images from the chase cam behind the quad.
        this would be used for creating visualizations of the quad's trajectory in the scene
        ...
        for pose in poses:
            ...
            camera_position[0] = aabb[0] + (aabb[3] - aabb[0]) / 2
            camera_position[1] = max(
                (aabb[3] - aabb[0]) / 2, (aabb[5] - aabb[2]) / 2
            )  # camera_position[1] + 3
            camera_position[2] = aabb[2] + (aabb[5] - aabb[2]) / 2
    ...
    def render_top(self, aabb, draw_traj=True):
        """
        move the quad and retrieve third-person view images from the chase cam behind the quad.
        this would be used for creating visualizations of the quad's trajectory in the scene
    ...
    def sample_path_2p(self, curr_loc, goal_loc):
        found_path = False
        cl = np.copy(curr_loc)
        cl[2] = cl[1]
        while not found_path:
