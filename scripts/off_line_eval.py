...
def run_eval(scene, run_idx):
    ...
    datapath = f"/home/siminghe/code/active-perception-game/data/habitat_collection/{run_idx}/train/data0.npz"
    ...
    for it, (depth_img, sem_img, pose) in enumerate(zip(depths, semantics, cam_poses)):
        ...
        if it > 39 and (it - 39) % 30 == 0:
            det_sem_objs.append(
                update_sem_step(sem_grids, gt_obj_locs, det_dist_thresh=det_dist_thresh)
            )
            steps_taken += 1
    ...
    for i in range(len(sem_grids)):
        # print(
        #     "Semantic class {}: {} detected, {} gt".format(
        #         i + 1, det_sem_objs[-1][i], gt_objs_num[i]
        #     )
        # )
        total_num += gt_objs_num[i]

    print("Total Exist: ", total_num)
    ...
    for i in range(len(total_num_det_step) - 1):
        total_num_det_step[i + 1] = max(
            total_num_det_step[i], total_num_det_step[i + 1]
        )
    total_num_det_step = np.insert(total_num_det_step, 0, 0)

    np.save(
        "results/eval_" + scene + run_idx + ".npy",
        total_num_det_step,
    )

    print(total_num_det_step)


# for scene in scenes:
# for i in range(1, 4, 1):
# indices = [
# "529_good_model_improved_1",
# "529_good_model_improved_2",
# "529_good_model_improved_3",
# "529_good_model_orig_1",
# "529_good_model_orig_2",
# "529_good_model_orig_3",
# "250_good_model_improved_1",
# "250_good_model_improved_2",
# "250_good_model_improved_3",
# "250_good_model_orig_1",
# "250_good_model_orig_2",
# "250_good_model_orig_3",
# "036_vg_vg_improved_1",
# "036_vg_vg_improved_2",
# "036_vg_vg_improved_3",
# "036_vg_vg_orig_1",
# "036_vg_vg_orig_2",
# "036_vg_vg_orig_3",
# ]
...
# for i in indices:
run_eval("102816036", "036_final_video_improved_3")
# run_eval("102344280", 4)
