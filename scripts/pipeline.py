...
class ActiveNeRFMapper:
    ...
    def training_single_image(self, image_idx, steps=30):
        ...
        for step in range(steps):
            ...
            for model_idx, (
                ...
                max_depth = torch.max(dep).detach().item()
                loss_dep = F.smooth_l1_loss(
                    depth / max_depth, dep.unsqueeze(1) / max_depth
                )
                ...
                if not torch.isnan(loss_rgb):
                    loss += loss_rgb
                if not torch.isnan(loss_dep):
                    loss += loss_dep
                if not torch.isnan(loss_sem):
                    # if loss > 0.1:
                    #     loss = loss + (loss_sem / loss_sem.detach().item() / 10)
                    # else:
                    #     loss = loss + loss_sem
                    loss += loss_sem / 1000
                if loss == 0:
                    print("loss is zero")
                    continue
                # loss = loss_rgb + loss_dep + loss_sem / 10
    ...
    def nerf_training(
        ...
        for step in tqdm.tqdm(range(steps)):
            ...
            for model_idx, (
                ...
                if c < 0.3 and not final_train and not initial_train:
                ...
                # torch.nn.utils.clip_grad_norm_(radiance_field.parameters(), 10.0)
        ...
        while flag and step < self.config_file["planning_step"]:
            ...
            if self.policy_type == "uncertainty":
                ...
                for i in tqdm.tqdm(range(self.config_file["num_traj"])):
                    ...
                    uncertainties_orig.append(uncertainty)
                ...
                best_orig = np.argmax(np.array(uncertainties_orig))
                self.alternative_paths[0].append(N_sample_traj_pose[best_orig])

                best_index = np.argmax(np.array(uncertainties))
                self.alternative_paths[1].append(N_sample_traj_pose[best_index])
                ...
                # st()
                # collision_idx = -1
                # for idx, dep_img in enumerate(sampled_depth_images):
                #     if np.min(dep_img) < 0.05:
                #         collision_idx = idx
                #         break

                # if collision_idx > 0:
                #     sampled_images = sampled_images[:collision_idx]
                #     sampled_depth_images = sampled_depth_images[:collision_idx]
                #     sampled_sem_images = sampled_sem_images[:collision_idx]
                #     copy_traj[best_index] = copy_traj[best_index][:collision_idx]
                #     N_sample_traj_pose[best_index] = N_sample_traj_pose[best_index][
                #         :collision_idx
                #     ]
                #     self.render(copy_traj[best_index][:-19][::5])
                #     self.current_pose = copy_traj[best_index][-1]
                #     unc_idx = unc_idx[unc_idx <= collision_idx]
                # else:
                self.render(copy_traj[best_index][:-19][::5])
                self.current_pose = copy_traj[best_index][-1]
                ...
                self.render(N_sample_traj_pose[best_index][:-19][::5])
            ...
            ax.legend()
            fig.savefig(self.save_path + "/loss_improved.png")
            fig.clf()
            plt.close(fig)
            # if step >= 5:
            #     if (
    ...
    def pipeline(self):
        ...
        self.nerf_training(3000, initial_train=True)
        ...
        # )
        self.nerf_training(3000, final_train=True, planning_step=-10)
        ...
        np.save(
            self.save_path + "/alternative_path.npy",
            np.array(self.alternative_paths, dtype=object),
            allow_pickle=True,
        )
